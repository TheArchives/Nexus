    ...
    def commandSetJail(self, parts, fromloc, overriderank):
        ...
        for id, zone in self.client.world.rankzones.items():
            if zone[0] == zonename:
                exists = True
        if not exists:
            self.client.sendErrorMessage("Zone '%s' doesn't exist in this world!" % zonename)
            return
        self.prepJail()
        jail = self.loadJail()
    ...
    def commandJail(self, parts, fromloc, overriderank):
        ...
        if len(parts) == 3:
            if not parts[2].isdigit():
                self.client.sendErrorMessage("You must specify a positive numerical value for minutes!")
                return
            time = int(parts[2])
