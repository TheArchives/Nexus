...
from core.blocktracker import Tracker
...
class World(object):
            ...
            for i in range(1000):
                ...
                try:
                    ...
                    # Or a message for the admins
                        ...
                        self.factory.queue.put((self, TASK_ADMINMESSAGE, (255, self, task[1] % {"id": self.id})))
                    # ???
                    else:
    ...
    def isOwner(self, name):
        return (name.lower() in self.owner)
        
    def isOp(self, name):
        return (name.lower() in self.ops)
        
    def isBuilder(self, name):
        return (name.lower() in self.builders)
